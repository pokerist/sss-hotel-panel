...
function startMockPMS() {
  ...
  app.use((req, res, next) => {
    logger.info(`Mock PMS: ${req.method} ${req.path}`, {
      query: req.query,
      body: req.body
    });
    next();
  ...
  app.use('*', (req, res) => {
    logger.warn(`Mock PMS: Unimplemented endpoint ${req.method} ${req.path}`);
    res.status(404).json({
  ...
  app.use((error, req, res, next) => {
    logger.error('Mock PMS Server Error:', error);
    res.status(500).json({
  ...
  mockServer = app.listen(port, () => {
    logger.info(`Mock PMS Server started on port ${port}`);
    logger.info('Available endpoints:');
    logger.info('  - GET /guest/v0/guests');
    logger.info('  - GET /reservation/v0/reservations');
    logger.info('  - GET /folio/v0/folios');
    logger.info('  - GET /rooms/:roomNumber/status');
    logger.info('  - GET /test/error/:code (for testing)');
  ...
  process.on('SIGTERM', () => {
    if (mockServer) {
      mockServer.close(() => {
        logger.info('Mock PMS Server closed');
      });
    }
  });
...
function stopMockPMS() {
  if (mockServer) {
    mockServer.close(() => {
      logger.info('Mock PMS Server stopped');
    });
    mockServer = null;
  }
}

module.exports = startMockPMS;
module.exports.stop = stopMockPMS;
